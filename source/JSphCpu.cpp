...
//FACKERNEL
void JSphCpu::GetKernelWendlandwithfac(float rr2, float drx, float dry, float drz
  , float &frx, float &fry, float &frz, float &fac)const
{
  const float rad=sqrt(rr2);
  const float qq=rad/H;
  //-Wendland kernel.
  const float wqq1=1.f-0.5f*qq;
  const float wqq2=wqq1*wqq1;
  //const fac=Bwen*qq*wqq2*wqq1/rad; //-Kernel derivative (divided by rad).

  fac=Bwen*qq*wqq2*wqq1/rad; //-Kernel derivative (divided by rad).
  frx=fac*drx; fry=fac*dry; frz=fac*drz;
  const float wqq=2.f*qq+1.f;
  //wab=Awen*wqq*wqq2*wqq2; //-Kernel.
}  //<vs_innlet_end>
    ...
    for(int z=zini;z<zfin;z++){
      ...
      for(int y=yini;y<yfin;y++){
        ...
        for(unsigned p2=pini;p2<pfin;p2++){
          ...
          if(rr2<=Fourh2 && rr2>=ALMOSTZERO){
            //-Cubic Spline, Wendland or Gaussian kernel.
            float frx,fry,frz,fac;
            if(tker==KERNEL_Wendland)GetKernelWendlandwithfac(rr2,drx,dry,drz,frx,fry,frz,fac);
            else if(tker==KERNEL_Gaussian)GetKernelGaussian(rr2,drx,dry,drz,frx,fry,frz);
            else if(tker==KERNEL_Cubic)GetKernelCubic(rr2,drx,dry,drz,frx,fry,frz);
...
//edited Artificial Visco
              ...
              if(compute){
                  ...
                  //const float NU1 = 0.801e-3;	//dYNAMIC x10^-3
                  const float NU1 = 0.798e-6;	// Kinematic vis x10^-6 WATER
                  //const float NU1 = 73e-6;	// Kinematic vis x10^-6 HONEY
                 ...
                 // const float mu_ij_Vol = 1 * NU1 * (va*va+ vb*vb);
                  ...
                  const float A =(q-1)/H;
                  const float dfkernel = 140* A * A * A * 3.14159265 * q;
                  ...
                  acep1.x+=visc_factor_x*dfkernel; acep1.y+=visc_factor_y*dfkernel; acep1.z+=visc_factor_z*dfkernel;
